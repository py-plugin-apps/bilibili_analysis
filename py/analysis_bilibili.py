...
async def video_detail(url, **kwargs):
    try:
        ...
        title = f"\n标题：{res['title']}\n"
        cover = res["pic"] if analysis_display_image else ""
        ...
        if len(desc_list) > 4:
            desc = desc_list[0] + "\n" + desc_list[1] + "\n" + desc_list[2] + "……"
        msg = [cover, title, tname, stat, desc]
        return msg, vurl
    except Exception as e:
...
async def bangumi_detail(url, time_location):
    try:
        async with aiohttp.request(
            ...
            if not res:
                return None, None
        cover = res["cover"] if analysis_display_image else ""
        ...
        if time_location:
            time_location = time_location[0].replace("&amp;", "&")[3:]
            vurl += f"?t={time_location}"
        msg = [cover, title, index_title, desc, style, evaluate]
        return msg, vurl
    except Exception as e:
...
async def live_detail(url):
    try:
        ...
        if live_status:
            ...
            player = ""
        msg = [cover, title, up, watch, tags, player]
        return msg, vurl
    except Exception as e:
...
async def dynamic_detail(url):
    try:
        ...
        if images:
            images = [i.get("img_src") for i in images]
        else:
